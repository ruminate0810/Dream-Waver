...
// ─── Claw ──────────────────────────────────────────────────────────────
//
// General AI worker (Genspark-style). A ToolCallAgent loop plans sub-tasks,
// researches (web_search), and writes a markdown report artifact. Same WS
// event surface as slides/games via AgentSessionProvider; the report body
// rides GET /claw/{id}/artifact (never the WS — claw.artifact.updated is
// just a version notification).

export type ClawTask = {
  title: string;
  role?: string; // v2: assigned worker (coordinator/researcher/engineer/designer/writer/producer)
  status: "pending" | "doing" | "done" | "skipped";
};

export type ClawFigure = {
  url: string;
  caption?: string;
};

export type ClawDeck = {
  title?: string;
  slide_count?: number;
...
export type ClawVideo = {
  /** Servable mp4 (image-to-video output). */
  url: string;
  /** Source figure URL — used as the <video> poster. */
  poster?: string;
  caption?: string;
  resolution?: string;
  duration?: number;
...
export type CreateClawResponse = {
  job_id: string;
  session_id: string;
  events_url: string;
};

/** One playable mini-game in the work package (V26 批二). The HTML lives
 *  with the games vertical; play_url serves it raw for an iframe. */
...
export type ClawRun = {
  job_id: string;
  session_id: string;
  status: "running" | "awaiting_input" | "finished" | "error";
  prompt: string;
  title?: string;
  /** Authoritative plan checklist — present once plan_tasks has run.
   *  A cold page load draws the checklist from this without WS replay. */
  plan?: ClawTask[];
  /** Bumps on every write_document; 0 / undefined means no report yet. */
  artifact_version?: number;
  /** Set once a report exists. Same-origin GET → text/markdown. */
  artifact_url?: string;
  /** Work-package figures generated by the designer worker. */
  figures?: ClawFigure[];
  /** Work-package videos (image-to-video) generated by the videographer. */
  videos?: ClawVideo[];
  /** Work-package slide deck generated by the producer worker. */
  deck?: ClawDeck;
  /** Playable mini-games generated by the producer worker (V26 批二). */
  games?: ClawGame[];
  /** Set when status === "awaiting_input": the adaptive clarification questions. */
...
export async function createClawRun(prompt: string): Promise<CreateClawResponse> {
  const res = await fetch("/api/v1/claw", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ prompt }),
  });
  return unwrap<CreateClawResponse>(res);
}

export async function getClawRun(id: string): Promise<ClawRun> {
  const res = await fetch(`/api/v1/claw/${id}`);
  return unwrap<ClawRun>(res);
}
...
/** URL the ArtifactPanel / download link points at. */
export function clawArtifactURL(jobId: string): string {
  return `/api/v1/claw/${jobId}/artifact`;
}

/** Fetch the raw markdown report + its version. Does NOT go through
 *  unwrap — the endpoint serves text/markdown directly with the version
 *  in the X-Artifact-Version header. Returns null body on 404 (not ready). */
export async function fetchClawArtifact(
  jobId: string,
): Promise<{ markdown: string; version: number } | null> {
  const res = await fetch(clawArtifactURL(jobId), { cache: "no-store" });
  if (res.status === 404) return null;
  if (!res.ok) throw new ApiError(`artifact fetch failed: ${res.status}`, res.status);
  const markdown = await res.text();
  const version = Number(res.headers.get("X-Artifact-Version") ?? "0");

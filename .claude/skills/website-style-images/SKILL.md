...
**颜色多样性验证：** 提取完成后，检查 primary/secondary/accent 是否为 3 个不同颜色。如果不足 3 色，提示用户手动指定颜色或降低聚类阈值重试。
...
**成本预估:** N 种图片类型 × 1 次 API 调用 = N 次调用，约 N×0.75 分钟
...
- [ ] 无意外文字或水印
- [ ] 图片类型的构图符合平台要求
- [ ] 品牌个性（brand personality）在图片中可感知

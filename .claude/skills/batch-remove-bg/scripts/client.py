"""NewportAI client for background removal."""
...
class NewportAIClient:
    """Client for the NewportAI async image editing API (remove_background only)."""
    ...
    def remove_background(self, image_url: str) -> TaskResult:
        """Remove background from image."""
        param_key = self.endpoints.get("remove_background", {}).get("param_key", "url")
        return self.submit_and_wait("remove_background", {param_key: image_url})


def estimate_cost(count: int = 1) -> dict:
    """Estimate API cost for remove_background operations."""
    return {
        "actions": ["remove_background"],
        "images": count,
        "api_calls": count,
        "estimated_minutes": round(count * 0.75, 1),
    }

"""Simplified NewportAI client for colorize operations."""
...
class NewportAIClient:
    ...
    def colorize(self, image_url: str) -> TaskResult:
        """Colorize a black-and-white image."""
        param_key = self.endpoints.get("colorize", {}).get("param_key", "url")
        return self.submit_and_wait("colorize", {param_key: image_url})


def estimate_cost(count: int = 1) -> dict:
    """Estimate API cost for colorize operations.

    Each API call takes ~30-60 seconds.
    """
    return {
        "actions": ["colorize"],
        "images": count,
